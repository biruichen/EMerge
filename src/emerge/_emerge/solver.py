...
class MatrixType(Enum):
    ...
    The matrix type determines which solvers and preconditioners are valid:
      - SPD:                CG, CHOLMOD, MINRES
      - SYMMETRIC:          MINRES, LDL^T, symmetric direct solvers
      - HERMITIAN:          CG (complex), MINRES
      - HPD:                CG (complex), CHOLMOD (complex)
      - COMPLEX_SYMMETRIC:  BiCGSTAB, GMRES, direct solvers (not CG)
      - UNSYMMETRIC:        BiCGSTAB, GMRES, LU-based direct solvers
    """

    SPD = 0  # real, symmetric positive definite (thermal, structural)
    SYMMETRIC = 1  # real, symmetric indefinite (saddle-point, mixed formulations)
    HPD = 2  # complex, Hermitian positive definite
    HERMITIAN = 3  # complex, Hermitian indefinite
    COMPLEX_SYMMETRIC = 4  # complex symmetric, A = A^T but A != A^H (EM wave equation)
    UNSYMMETRIC = 5  # general, no structure (coupled multiphysics, convection)
    UNKNOWN = 6
...
def classify_matrix(A: csc_matrix, tol: float = 1e-10) -> MatrixType:
    """Classify a sparse matrix type as cheaply as possible.

    Checks are ordered from cheapest to most expensive,
    short-circuiting as soon as the type is determined.
    """
    n, m = A.shape
    if n != m:
...
class SmartARPACK(EigSolver):
    ...
    def eig(
        ...
        for i, q in enumerate(qs):
            ...
            sigma = sign * ((q * target_k0) ** 2)
            eigen_values, eigen_modes = eigs(A, k=nmodes, M=B, sigma=sigma, which=which)
            eigen_values, eigen_modes = self.reduct_solutions(
...
class SolveRoutine:
    ...
    def __init__(self, thread_nr: int = 0, proc_nr: int = 0):
        ...
        self.precon: Preconditioner = ICCPrecon()
