...
def fK_EA(x: np.ndarray, y: np.ndarray, z: np.ndarray, theta: float, phi: float, psi: float, k0: float, origin: tuple[float,float,float]) -> np.ndarray:
...
class BackgroundField(Saveable):
    ...
    def k(self, x: np.ndarray, y: np.ndarray, z:np.ndarray) -> np.ndarray:
        if self.definition == 'EA':
            ...
            raise ValueError(f'Unsupported spherical coordinate definition {self.definition}')
