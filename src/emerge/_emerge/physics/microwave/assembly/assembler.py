...
class Assembler:
    ...
    def assemble_freq_matrix(self, 
        ...
        if len(robin_bcs) > 0:
            ...
            # We compute the FEM matric contributions using Gaussian quadrature.
    ...
    def assemble_scattering_matrix(self, 
        ...
        if cache_matrices and not is_frequency_dependent and self.cached_matrices is not None:
            ...
            # OTHERWISE, COMPUTE
            logger.debug('Assembling matrices')
            matrix_stiff_coo, matrix_mass_coo, cscmap = tet_mass_stiffness_matrices(field, er, ur, self.cached_cscmap)
            self.cached_cscmap = cscmap
            self.cached_matrices = (matrix_stiff_coo, matrix_mass_coo)
            
        
        # COMBINE THE MASS AND STIFFNESS MATRIX
        matrix_fem: csc_matrix = self.cached_cscmap.to_csc(matrix_stiff_coo - matrix_mass_coo*(K0**2))
        ...
        simjob = SimJob(matrix_fem, background_fields, K0*299792458/(2*np.pi), True)
    ...
    def assemble_eig_matrix(self, field: Nedelec2, 
        ...
        matrix_stiff, matrix_mass, cscmapping = tet_mass_stiffness_matrices(field, er, ur)
        ...
        pec_ids_set = set(pec_ids)
        solve_ids = np.array([i for i in range(matrix_stiff.shape[0]) if i not in pec_ids_set])
